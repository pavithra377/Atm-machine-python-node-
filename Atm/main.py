...
def new_account(event=None):
    ...
    def limitSize(*args):
        value = limit.get()
        if len(value) > 8: limit.set(value[:8])
    ...
    def click(event=None):
        ...
        user = NewAccount(first, last, father, CNIC, Username, middle)
        ...
        if not Full_name:
            ...
            window.bell()
            logging.warn('user entered invalid CARD NUMBER : {}'.format(CNIC))
            messagebox.showwarning('Failed', "Invalid CARD Number!")

        elif (Username == " ") or (Username.lower() == first.lower()) or (Username.lower() == last.lower()) or (Username.lower() == father.lower()):
            window.bell()
            logging.warn('user entered invalid CARD NUMBER : {}'.format(Username))
            messagebox.showwarning('Failed', "Invalid CARD NUMBER : {}. Please enter a unique CARD NUMBER.".format(Username))
    ...
    #---label for cnic-----
    l_cnic = Label(Frame_1, text="CARD NUMBER  : ", font="none 12 bold").grid(row=7, column=0, sticky=W)
    Label(Frame_1, text="(8 DIGITS)", font="none 12 bold").grid(row=7, column=3, sticky=W)

    #------empty label----------
    Label(Frame_1).grid(row=8, sticky=EW)

    #-----label for username------
    Label(Frame_1, text="RE-ENTER CARD NUM:", font="none 12 bold").grid(row=9, column=0, sticky=W)
